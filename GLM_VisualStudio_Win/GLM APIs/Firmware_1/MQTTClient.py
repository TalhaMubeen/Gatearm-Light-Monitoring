...
class MQTTClient(object):
    ...
    def __init__(self, configs = {}):
        try:
            ...
            self.__mqttClient__     = mqtt.Client(client_id=self.MY_ID, clean_session=True)
    ...
    def PublishLiveData(self, data):
        ...
        if self.IsConnected():
            ...
            if rc.rc ==  mqtt.MQTT_ERR_SUCCESS:
                self.__logger__.log(self.__logger__.DEBUG, 'Message Published Successfully Topic = ' + topic +' Payload = ' + data )
                ret = True
            else:

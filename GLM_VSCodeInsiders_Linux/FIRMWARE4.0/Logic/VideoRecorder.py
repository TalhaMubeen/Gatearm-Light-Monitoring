...
class VideoRecorder(object):
	...
	class StreamingHandler(server.BaseHTTPRequestHandler):
		...
		def do_GET(self):
			if self.path == '/':
				...
				try:
					while True:
						with Logic.GetVideoRecorder().output.condition:
							Logic.GetVideoRecorder().output.condition.wait()
							frame = Logic.GetVideoRecorder().output.frame
					...
					Logic.GetLogger().log(Logic.GetLogger().ERROR, 'Failed to Init WEB STREAM')
					pass
			else:
				self.send_error(404)
				self.end_headers()
	...
	def __init__(self, configs):
		try:
			...
			self.PAGE = PAGE.replace("#",self.MY_ID )
			...
			try:
				...
				self.__picam__.rotation = self.flip

...
class MQTTDataProcessor(object):
    ...
    def __init__(self, configs={}):
        try:
            ...
            self.SendPeriodicData = Logic.PeriodicTimer(5 * 60, self.SendPeriodicLiveData)
            self._mqttClientConnectTimer = Logic.PeriodicTimer(1, Logic.GetMQTTClient().Connect)
    ...
    def Start(self):
        """Registers all the necessary callbacks. Starts periodic live timer, MQTT Client and sends startup live
        packet """
        ...
        self._mqttClientConnectTimer.start()
    ...
    def MqttEIPDataCallback(self, str_data=""):
        """Register this function with MQTTClient callback function to received data over live topic"""
        jsonData = {}

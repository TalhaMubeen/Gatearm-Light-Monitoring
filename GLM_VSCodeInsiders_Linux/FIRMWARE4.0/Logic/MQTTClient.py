...
class MQTTClient(object):
	...
	def Instance():
		...
		"""
		return MQTTClient.__instance__
	...
	def __init__(self, configs = {}):
		try:
			...
			self.__mqttClient__     = mqtt.Client(client_id=MacAddress, clean_session=False)
			...
			if MQTTClient.__instance__ is None:
				MQTTClient.__instance__ = self
	...
	def Reconnect(self):
		try:
			...
			self.logger.log(self.logger.ERROR, 'Failed to Reconnect MQTT Client')

	def __On_Disconnect__(self, client, userdata, rc):
		self.Reconnect()
		if rc != 0:
			self.logger.log(self.logger.ERROR, 'Unexpected disconnection from MQTT Server')
		else:
			pass
	...
	def RegisterMessageRecieveCallback(self, topic, function):
		...
		if type(topic) is str :
			if self.__msg_rcv_callback__.get(topic) == None:
				...
				self.logger.log(self.logger.ERROR, 'callback to topic ' + topic + ' is already registered')
		else:
			self.logger.log(self.logger.ERROR, 'topic type must be string to register callback function')
	...
	def MakeSubscriptionMap(self):
		#Customer/Product/MY_ID/command
	...
	def __Subscribe__(self):
		...
		if len(self.__subscription_map__):
			...
			#Sending IP Address twice on startup
			Logic.GetMQTTDataProcessor().SendPeriodicLiveData()
			Logic.GetMQTTDataProcessor().SendPeriodicLiveData()
			self.logger.log(self.logger.DEBUG,  "Starting Sending Periodic DATA" )
			pass

	def __On_Subscribe__(self,client, userdata, mid, granted_qos):
		"""called by mqttclient on subscription to any topic.
			must be called after calling mqtt connect connect """
		self.logger.log(self.logger.DEBUG,  self.currentSubTopic + " Subscribed")
	...
	def __On_Connect__(self, client, userdata, flags, rc):
		...
		if rc == 0: #connection is successful
			...
			self.logger.log(self.logger.DEBUG,'MQTT Client Connected Successfully')
			...
			self.logger.log(self.logger.ERROR, 'Failed to Connect MQTT Client, Reason : ' + str(rc))
	...
	def PublishLiveData(self, data):
		...
		if self.IsConnected():
			...
			if rc.rc ==  mqtt.MQTT_ERR_SUCCESS:
				self.logger.log(self.logger.DEBUG, 'Message Published Successfully Topic = ' + topic +' Payload = ' + data , True)
				ret = True
			else:
				self.logger.log(self.logger.DEBUG, 'Failed to Publish Data | Topic = ' + topic +' Payload = ' + data )
	...
	def Publish(self, data):
		...
		if self.IsConnected():
			...
			self.__MessageQueue__['status'] = data
		return ret

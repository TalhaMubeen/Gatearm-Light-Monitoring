...
class HttpClient(object):
	...
	def Instance():
		...
		"""
		return HttpClient.__instance__
	...
	def __init__(self, configs):
		try:
			...
			if HttpClient.__instance__ is None:
				HttpClient.__instance__ = self
		except:
					...
					try:
						...
						if int(ret.status_code) == 200:
							...
							self.logger.log(self.logger.ERROR,
											'Failed to Upload ' + fileType + ' , HTTP Return Code ' + str(
												ret.status_code))
	...
	def PostImages(self, imagePath):
		...
		p = ParallelProcess(target=self.__ExecuteParallel__, args=('imagefile', self.ImageUploadUrl, imagePath, 'post'))
		self.Jobs.append(p)

	def PostVideo(self, videoPath):
		self.logger.log(self.logger.INFO, 'Requesting to Upload Video : ' + videoPath)
		p = ParallelProcess(target=self.__ExecuteParallel__, args=('videofile', self.VideoUploadURL, videoPath, 'post'))
		self.Jobs.append(p)

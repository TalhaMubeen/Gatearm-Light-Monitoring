...
class VideoRecordingProcessor(object):
    ...
    def __init__(self, configs):
        try:
            ...
            self.__UploadVideoTimer__ = Logic.PeriodicTimer(timeout * 60, self.UploadVideoOnTimer)
        ...
        if ret == True:
            ...
            if self.isWaitingUpload:
                ...
                self.logger.log(self.logger.ERROR,
                                'Stop Recording Packet Recieved: No Video Recording | No Pending Upload')
        return
    ...
    def OnVideoRecorded(self, path, TimeStamp):
        ...
        if self.StopRecordingPktRcvd == True:
            ...
            self.logger.log(self.logger.INFO,
                            'Video Upload API Waiting For STOP RECORDING PACKET from EIP ' + self.EIP_ID)
    ...
    def Process(self):
        ...
        elif Logic.GetLedDetector().inQueue():

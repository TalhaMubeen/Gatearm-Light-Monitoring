    ...
    def ResetDetectionState(self):
        ...
        for i in range(len(blinks)):
            self.logger.log(self.logger.INFO,
                            'Blink Count of Recording ' + self.inputFileName + ' : Led =>' + str(i) + ' : ' + str(
                                blinks[i][1][1]))
    ...
    def ProcessVideoFrame(self):
        ...
        for fcount in range(6):
            ...
            if ret1 == True:
                self.frameCount += 1
                self.logger.log(self.logger.INFO,
                                "Processing Frame No: " + str(self.frameCount) + ' Out of ' + str(self.TotalFrame),
                                True)
                ...
                for i in range(len(LightLoc)):
                    ...
                    if ret == False and not self.foundOneLED[i]:
                        ...
                        yloc = 60 + (30 * i)
                        if count > 0:
                            cv2.putText(frame1, 'LIGHT ' + str(i + 1) + ' : ' + str(count), (5, yloc),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                del LightLoc
                cv2.imwrite(self.TempDirPath + "image" + str(self.frameCount) + ".jpg", frame1,

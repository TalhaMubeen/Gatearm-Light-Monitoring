...
class LocalLogger(object):
    ...
    def __init__(self, FileName, ApiName, configs):
        """
        ...
        try:
            ...
            self.__Log_Dir__        = configs['GLM']['DIAGNOSTICS']['LOG_PATH']
            self.isDeatiledLog      = configs['GLM']['DIAGNOSTICS']['DetailLogEnabled']
    ...
    def __get_logger_object__(self):
        ...
        formatter = logging.Formatter("%(asctime)s.%(msecs)03d-%(levelname)s[%(name)s:%(lineno)d] : %(message)s" , datefmt='%b-%d-%y %H:%M:%S')
